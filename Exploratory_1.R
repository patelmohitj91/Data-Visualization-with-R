...
# Lets dig deeper into the above bar chart - FACETS - not that impressive

ggplot(data=Top_Crime_df,aes(AREA.NAME,fill=CrmCd.Desc))+geom_bar(color="Black")+xlab("Area")+ylab("Crime Count")+facet_grid()
?facet_grid
...
# Frequency vs Time based occurance of most common crime in high risk areas 
ggplot(data=Top_Crime_df,aes(TIMECat,fill=CrmCd.Desc))+geom_bar(colour="Black")+xlab("TIME CATEGORY")+ylab("CRIME-FREQUENCY")+ggtitle("Frequency of common crimes in high risk areas")


# Frequency vs Time based occurance of most common crime in all areas
